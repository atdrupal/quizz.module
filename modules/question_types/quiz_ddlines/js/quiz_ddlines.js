...
(function ($, Drupal, Raphael, QuizElementList, QuizUtil, Answers, QuizPointer) {
  ...
  function Engine() {
    ...
    this.init = function (container_class, node_id) {
      ...
      // Add a specific ID and class to div, so CSS may be set only for our case:
      ...
      if (this.isEditMode()) {
        ...
        $(this.getSelector()).click(function (event) {
          ...
          if (targetIsSVG || targetIsImage || (self.helptext != null && event.target.nodeName === 'tspan')) {
            // Remove help text:
  ...
  function QuizLabel(parent, color, id, text, x, y) {
    ...
    var edit_mode = function (event) {
      ...
      this.editingDone = function () {
        self.parent.setText($editButton.val()); // Save the text
        $editButton.remove(); // Remove the input field
        self.parent.redrawConnection();
      ...
      $editButton.keyup(function (event) {
        $('#quiz-ddlines-toolpane label > em').html($editButton.val()); // Update toolpane
      });
    ...
    var focus_mode = function () {
      ...
      $toolPane.css({'z-index': 10001, 'border-color': self.color});
      ...
      $('#ddlines-close-panel').click(function () {
        $colorContainer.hide(); // Hide colorwheel
        QuizElementList.updateForm(); // Update form for saving
        $("#quiz-ddlines-toolpane").fadeOut();
      });
    };
